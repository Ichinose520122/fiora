...
COPY deploy/docker-deps.cjs deploy/docker-deps.cjs
RUN node deploy/docker-deps.cjs && yarn install --ignore-engines --ignore-scripts --non-interactive --network-timeout 120000 && yarn cache clean
...
COPY tsconfig.json ./
RUN node deploy/docker-deps.cjs --links && touch .env
ENV NODE_ENV=production
ENV NODE_OPTIONS=--max-old-space-size=1600
RUN cd packages/web && ../../node_modules/.bin/webpack --config build/webpack.docker.js && cp -r dist/fiora/. ../server/public/
