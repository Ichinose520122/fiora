...
export default function PixivAccount({ close }: { close: () => void }) {
    ...
    const button = (label: string, press: () => void) => <TouchableOpacity disabled={busy} onPress={press} style={{ paddingVertical: 14, opacity: busy ? 0.4 : 1 }}><Text style={{ color: '#526b98' }}>{label}</Text></TouchableOpacity>;
    return <Modal visible animationType="slide" onRequestClose={close}><SafeAreaView style={{ flex: 1, padding: 20, backgroundColor: '#eaf0f6' }}>
        ...
        <Text>使用 Pixiv 官方网页登录取得的 PHPSESSID。仅站点管理员可管理此账号，图片由服务器获取后发送。</Text>
        <TextInput value={session} onChangeText={setSession} placeholder="PHPSESSID" secureTextEntry autoCapitalize="none" autoCorrect={false} maxLength={4096} style={{ backgroundColor: 'white', borderRadius: 8, padding: 12, marginVertical: 12 }} />
