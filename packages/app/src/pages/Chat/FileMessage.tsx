...
export default function FileMessage({ message }: { message: Message }) {
    ...
    return <TouchableOpacity disabled={!url || message.loading} onPress={() => {
        ...
        <View style={{ borderRadius: 13, backgroundColor: '#ffffff70', padding: 10, marginRight: 9 }}><Ionicons name="document-text-outline" size={28} color="#405c81" /></View>
        <View style={{ flexShrink: 1 }}><Text numberOfLines={2} style={{ color: '#243850', fontWeight: '600' }}>{file.filename || '文件'}</Text><Text style={{ color: '#526780', fontSize: 12, marginTop: 5 }}>{((file.size || 0) / 1024 / 1024).toFixed(2)} MB · {message.loading ? '发送中' : '打开 / 下载'}</Text></View>
    </TouchableOpacity>;
}
