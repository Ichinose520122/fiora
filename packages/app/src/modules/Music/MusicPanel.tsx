...
export default function MusicPanel() {
    ...
    return <Modal visible={music.panel} animationType="slide" onRequestClose={music.close}>
        <SafeAreaView style={{ flex: 1, backgroundColor: '#f0f3fb' }}>
            ...
            <View style={styles.tabs}>{[['search', '点歌'], ['queue', `队列 ${room?.queue.length || 0}`], ['playlist', '歌单'], ['account', '账号']].map(([tab, label]) => <TouchableOpacity key={tab} onPress={() => music.setPanelTab(tab)} style={[styles.tab, panelTab === tab && styles.activeTab]}><Text style={{ color: panelTab === tab ? '#596fa8' : '#98a4bc', fontSize: 13, fontWeight: '600' }}>{label}</Text></TouchableOpacity>)}</View>
            {panelTab !== 'account' && <LinearGradient colors={['#e5eafa', '#f2eaf7', '#eaf1fc']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.now}>
