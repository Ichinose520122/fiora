...
export default function MusicPlayer() {
    ...
    return <GlassView intensity={42} tint="light" style={styles.card}>
        <View style={styles.row}>
            <TouchableOpacity onPress={music.open} accessibilityLabel="打开音乐面板">
                <Animated.View style={[styles.disc, { transform: [{ rotate: spin.interpolate({ inputRange: [0, 1], outputRange: ['0deg', '360deg'] }) }] }]}>
                    {track.cover ? <Animated.Image source={{ uri: assetUrl(track.cover) }} style={styles.cover} /> : <MusicIcon color={theme.color('white')} size={24} />}<View style={styles.hole} />
                </Animated.View>
            </TouchableOpacity>
            <TouchableOpacity onPress={music.open} style={{ flex: 1, minWidth: 0, marginLeft: 12 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 5 }}><View style={[styles.dot, { backgroundColor: music.listening ? theme.color('#74b3a7', 'backgroundColor') : theme.color('#adb6c7', 'backgroundColor') }]} /><Text style={styles.status}>{room.paused ? '已暂停' : music.listening ? '正在一起听' : '点击加入收听'} · {room.listeners} 人</Text></View>
            </TouchableOpacity>
            <TouchableOpacity onPress={music.open} style={styles.queue} accessibilityLabel="查看点歌队列"><Ionicons name="list-outline" size={20} color={theme.color('#7382a5')} /><Text style={styles.status}>{room.queue.length}</Text></TouchableOpacity>
        </View>
        ...
        {room.canControl ? <Slider accessibilityLabel="房间播放进度" minimumValue={0} maximumValue={Math.max(1, track.duration)} value={position} onSlidingComplete={(next) => { void music.act('seek', { position: next }); }} minimumTrackTintColor={theme.color('#97a6d4')} thumbTintColor={theme.color('#7484b8')} /> : <View style={styles.timeline}><View style={[styles.progress, { width: `${percent}%` }]} /></View>}
        ...
        <View style={[styles.row, { marginTop: 2 }]}>
            <TouchableOpacity onPress={music.listening ? music.leave : music.join} style={styles.join} accessibilityLabel={music.listening ? '退出收听' : '加入收听'}><Ionicons name={music.listening ? 'headset' : 'headset-outline'} size={17} color={theme.color('#5e6fa9')} /><Text style={styles.joinText}>{music.listening ? '收听中' : '一起听'}</Text></TouchableOpacity>
            <Slider accessibilityLabel="播放音量" style={{ flex: 1, minWidth: 30, height: 32 }} value={music.volume} minimumValue={0} maximumValue={1} onValueChange={music.setVolume} minimumTrackTintColor={theme.color('#8898cb')} thumbTintColor={theme.color('#7484b8')} />
            <Text style={styles.volume}>{Math.round(music.volume * 100)}%</Text>
            {room.canControl && <TouchableOpacity accessibilityLabel={room.paused ? '继续播放' : '暂停房间播放'} onPress={() => { void music.act(room.paused ? 'resume' : 'pause'); }} style={styles.control}><Ionicons name={room.paused ? 'play' : 'pause'} size={20} color={theme.color('#5e6fa9')} /></TouchableOpacity>}
            <TouchableOpacity accessibilityLabel={room.canControl ? '下一首' : '投票切歌'} onPress={() => { void music.act(room.canControl ? 'next' : 'vote'); }} style={styles.control}><Ionicons name="play-skip-forward" size={19} color={theme.color('#5e6fa9')} /></TouchableOpacity>
        </View>
