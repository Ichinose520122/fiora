...
export default function MusicAccount() {
    ...
    const field = { backgroundColor: theme.color('#ffffffb0', 'backgroundColor'), padding: 10, borderRadius: 7, marginVertical: 5 };
    const button = (label: string, press: () => void, disabled = false) => <TouchableOpacity disabled={busy || disabled} onPress={press} style={{ padding: 12, opacity: busy || disabled ? 0.45 : 1 }}><Text style={{ color: theme.color('#526b98', 'color') }}>{label}</Text></TouchableOpacity>;
    return <View>
        <Text style={{ marginVertical: 8 }}>网易云音乐账号（供服务器播放音乐）</Text>
        <Text style={{ color: theme.text }}>{!status ? '正在读取账号状态…' : status.loggedIn ? `已登录 ${status.nickname || ''}` : status.unverified ? '已保存，尚未验证' : '未登录'}</Text>
        {admin && status?.canManage && status.configured ? <>
            <TextInput style={[field, { backgroundColor: theme.input, color: theme.text, borderWidth: 1, borderColor: theme.border }]} placeholder="国家区号" keyboardType="number-pad" value={countryCode} onChangeText={setCountryCode} maxLength={4} />
            <TextInput style={[field, { backgroundColor: theme.input, color: theme.text, borderWidth: 1, borderColor: theme.border }]} placeholder="手机号" keyboardType="phone-pad" value={phone} onChangeText={setPhone} maxLength={15} />
            {button(cooldown ? `${cooldown} 秒后可重发` : '发送验证码', () => { void run('musicSendCode', { phone, countryCode }); }, cooldown > 0)}
            <TextInput style={[field, { backgroundColor: theme.input, color: theme.text, borderWidth: 1, borderColor: theme.border }]} placeholder="短信验证码" keyboardType="number-pad" value={captcha} onChangeText={setCaptcha} maxLength={8} autoComplete="sms-otp" />
            {button('验证码登录', () => { void run('musicLogin', { phone, countryCode, captcha }); })}
            <Text style={{ color: theme.text }}>短信无法登录时，可使用官方网页登录取得的 MUSIC_U。</Text>
            <TextInput style={[field, { backgroundColor: theme.input, color: theme.text, borderWidth: 1, borderColor: theme.border }]} placeholder="MUSIC_U（仅提交给本服务器）" value={cookie} onChangeText={setCookie} secureTextEntry autoCapitalize="none" autoCorrect={false} maxLength={4096} />
