...
export async function loadRoom(id: string) {
    if (!rooms.has(id)) {
        ...
        // Backfill fields added after older rooms were persisted.
...
export function snapshot(
    ...
    return {
        ...
        serverTime: Date.now(),
        listeners,
        votesNeeded: Math.max(1, Math.ceil(listeners / 2)),
        canControl,
    };
}
...
export async function nextTrack(room: MusicRoomState) {
    ...
    let attempts = Math.min(
        5,
        fromQueue ? room.queue.length : room.idlePlaylist.length,
    );
    ...
    while (attempts-- > 0) {
        ...
        try {
            ...
            // Silently skip unavailable/trial-only tracks and continue.
            room.notice = '';
        }
    }
}
...
export async function publish(room: MusicRoomState) {
    ...
    await MusicRoom.updateOne(
        { _id: room.roomId },
        { $set: { state: room, updatedAt: new Date() } },
        { upsert: true },
    );
    ...
    await Promise.all(
        Array.from(subscribers.entries())
            .filter(([, sub]) => sub.roomId === room.roomId)
            .map(async ([id, sub]) => {
                try {
                    assert(
                        sub.socket.data.user === sub.userId,
                    );
                    const access = await getLinkmanAccess(
                        sub.userId,
                        room.roomId,
                    );
                    sub.socket.emit(
                        ...
                        snapshot(
                            room,
                            !access.group ||
                                access.group.creator?.toString() ===
                                    sub.userId ||
                                !!sub.socket.data.isAdmin,
                        ),
                    );
                } catch (_) {
                    sub.socket.emit('musicAccessLost', {
                        roomId: room.roomId,
                    });
...
export function subscribe(
    ...
    assert(
        socket && socket.data.user === userId,
        '连接已失效',
    );
...
export function installMusic(io: Server) {
    ...
    const timer = setInterval(() => {
        rooms.forEach((room, id) => {
            ...
            const pendingQueue =
                !room.current && room.queue.length > 0;
            ...
            if (
                ...
                roomSerial
                    ...
                    .catch(() => undefined)
                    .finally(() => ticking.delete(id));
            }

            if (
                !room.current &&
                Date.now() - (touched.get(id) || 0) >
                    300000 &&
