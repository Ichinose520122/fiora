...
function createMusicAuth({ api, token, accountFile, legacyCookieFile }) {
    ...
    async function readAccount() {
        try { return JSON.parse(await fs.promises.readFile(accountFile, 'utf8')); }
        catch (error) {
            if (error.code !== 'ENOENT') throw error;
            let cookie = '';
            try { cookie = (await fs.promises.readFile(legacyCookieFile, 'utf8')).trim(); }
    ...
    async function save(account) {
        ...
        const temporary = accountFile + '.' + crypto.randomBytes(8).toString('hex') + '.tmp';
        try {
            await fs.promises.writeFile(temporary, JSON.stringify(account), { mode: 0o600, flag: 'wx' });
    ...
    function authorized(req) {
        ...
        return typeof token === 'string' && token.length >= 32 && typeof received === 'string' &&
    ...
    function validate(data) {
        if (typeof data.phone !== 'string' || !/^\d{5,15}$/.test(data.phone) ||
            typeof data.countryCode !== 'string' || !/^\d{1,4}$/.test(data.countryCode)) throw new Error('invalid_body');
    }
    function failure(error, fallback) {
        ...
        if ([502, 503].includes(Number(code)) && fallback === 'login_failed') return 'invalid_code';
        return fallback;
    }
    async function handle(req, res, url) {
        if (!url.pathname.startsWith('/auth/')) return false;
        if (!authorized(req)) { result(res, { ok: false }, 403); return true; }
        if (req.method !== 'POST') { result(res, { ok: false }, 405); return true; }
        try {
            ...
            if (url.pathname === '/auth/status') {
                ...
                const now = Date.now();
                sent.forEach((until, id) => { if (until <= now) sent.delete(id); });
                if (sent.has(key)) { result(res, { ok: false, error: 'rate_limit' }); return true; }
                sent.set(key, now + 60000);
                try {
                    const response = await api.captcha_sent({ phone: data.phone, ctcode: data.countryCode });
                    if (response.body?.code !== 200) throw response;
                    result(res, { ok: true });
                } catch (error) { result(res, { ok: false, error: failure(error, 'send_failed') }); }
            } else if (url.pathname === '/auth/login') {
                validate(data);
                if (typeof data.captcha !== 'string' || !/^\d{4,8}$/.test(data.captcha)) throw new Error('invalid_body');
                await serial(async () => {
                    let account;
                    try {
                ...
                result(res, { ok: true });
            } else result(res, { ok: false }, 404);
        } catch (_) { result(res, { ok: false, error: 'unavailable' }); }
        return true;
    }
    return { handle, readCookie: async () => (await readAccount()).cookie || '' };
}
